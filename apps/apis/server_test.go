...
func (s *fakeStore) Register(
	...
	if s.registerErr != nil {
		return authSession{}, "", s.registerErr
	}
...
func (s *fakeStore) Credentials(_ context.Context, email string) (user, string, error) {
...
func (s *fakeStore) Session(_ context.Context, token string) (authSession, error) {
	if token != s.token {
...
func testServer(store authStore) http.Handler {
	return newAPIServer(store, config{
...
func TestSignUpSetsSessionCookie(t *testing.T) {
	store := &fakeStore{
		session: authSession{User: user{ID: "user-1"}, Session: session{ID: "session-1"}},
		token:   "raw-session-token",
	}
	body := bytes.NewBufferString(`{"name":"Ada","email":"ADA@example.com","password":"password123"}`)
	request := httptest.NewRequest(http.MethodPost, "/api/auth/sign-up/email", body)
	response := httptest.NewRecorder()
...
func TestSignInRejectsInvalidPassword(t *testing.T) {
	...
	store := &fakeStore{
		account:      user{ID: "user-1", Email: "ada@example.com"},
		passwordHash: string(passwordHash),
	}
	body := bytes.NewBufferString(`{"email":"ada@example.com","password":"wrong-password"}`)
	request := httptest.NewRequest(http.MethodPost, "/api/auth/sign-in/email", body)
	response := httptest.NewRecorder()
...
func TestMeRequiresValidSession(t *testing.T) {
	store := &fakeStore{
		session: authSession{User: user{ID: "user-1", Name: "Ada"}},
		token:   "valid-token",
	}
...
func TestEmailConflict(t *testing.T) {
	store := &fakeStore{registerErr: errEmailUsed}
	body := bytes.NewBufferString(`{"name":"Ada","email":"ada@example.com","password":"password123"}`)
	request := httptest.NewRequest(http.MethodPost, "/api/auth/sign-up/email", body)
	response := httptest.NewRecorder()
